    ...
    <div className="max-w-2xl mx-auto">
      ...
      {editing ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6 dark:bg-gray-800 dark:border-gray-700">
          <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-100">{editing.id ? 'Editar Empresa' : 'Nueva Empresa'}</h3>

          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Logo</label>
            <div className="flex items-start gap-4">
              <div className="w-32 h-32 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center overflow-hidden bg-gray-50 dark:border-gray-600 dark:bg-gray-700">
                {preview ? (
                  <img src={preview} alt="Logo" className="max-w-full max-h-full object-contain" />
                ) : (
                  <span className="text-xs text-gray-400 dark:text-gray-500">Sin logo</span>
                )}
              </div>
              <div className="flex flex-col gap-2">
                <input ref={fileRef} type="file" accept="image/*" onChange={handleLogo} className="text-sm dark:text-gray-300" />
                {preview && (
                  <button type="button" onClick={removeLogo}
                    className="text-xs text-red-500 hover:text-red-700">Eliminar logo</button>
                )}
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Recomendado: PNG o JPG, máximo 500 KB</p>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-3">
          {companies.length === 0 && (
            <p className="text-sm text-gray-400 dark:text-gray-500 py-8 text-center">No hay empresas registradas. Agrega una para comenzar.</p>
          )}
          {companies.map(c => (
            <div key={c.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 flex items-center gap-4 dark:bg-gray-800 dark:border-gray-700">
              {c.logo_url ? (
                <img src={c.logo_url} alt="" className="w-12 h-12 object-contain rounded" />
              ) : (
                <div className="w-12 h-12 bg-gray-100 dark:bg-gray-700 rounded flex items-center justify-center text-gray-400 dark:text-gray-500 text-xs">Sin logo</div>
              )}
              <div className="flex-1 min-w-0">
                <p className="font-medium text-gray-800 dark:text-gray-100 truncate">{c.name || '(sin nombre)'}</p>
                {c.ruc && <p className="text-xs text-gray-500 dark:text-gray-400">RUC: {c.ruc}</p>}
              </div>
